...
export default function TransactionsList() {
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch(import.meta.env.VITE_BASE_URL+"/transaction/recentTransactions?limit=50")
      if (response.ok) {
  ...
  const handleDelete = async (id, e) => {
    ...
    try {
      const response = await fetch(import.meta.env.VITE_API_URL+`/transaction/deleteTransaction/${id}`, {
        method: "DELETE",
      })
